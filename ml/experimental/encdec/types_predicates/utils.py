...
class DataLoader():
    def __init__(self, input_file, args):
        ...
        raw_inputs, raw_targets, raw_types = reader.read_data()
        args.num_predicates = reader.num_predicates
        ...
        self.targets_p = np.zeros((sz, args.num_predicates, args.max_arity), dtype=np.bool)
        for i, (path, types) in enumerate(zip(raw_targets, raw_types)):
            ...
            arity = len(types['p0']) # just check one list to find arity
            assert arity <= args.max_arity, 'Too many arguments, increase max_arity'
            for j in range(args.num_predicates):
                self.targets_p[i, j, :arity] = types['p{0}'.format(j)]
        ...
        self.targets_p = np.split(self.targets_p, args.num_batches, axis=0)

        # reset batches
        self.reset_batches()

    def next_batch(self):
        x, l, n, e, y, p = next(self.batches)
        ...
        rn = np.transpose(n)
        re = np.transpose(e)
        rp = [p[:, i, :] for i in range(self.args.num_predicates)]

        return rx, rl, rn, re, y, rp

    def reset_batches(self):
        self.batches = zip(self.inputs, self.inputs_len, self.nodes, self.edges,
                           self.targets, self.targets_p)
