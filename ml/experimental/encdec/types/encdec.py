...
class Decoder(object):
    def __init__(self, args, initial_state, infer=False):
        ...
        with tf.variable_scope('decoder'):
            ...
            # feedforward network to handle refinement types
            def bool_ff_network(state, scope):
                with tf.variable_scope(scope):
                    w = tf.get_variable('w', [args.rnn_size, args.max_arity * 2])
                    b = tf.get_variable('b', [args.max_arity * 2])
                    out = tf.matmul(state, w) + b
                return tf.reshape(out, [-1, args.max_arity, 2])
            
            def num_ff_network(state, scope):
                with tf.variable_scope(scope):
                    w = tf.get_variable('w', [args.rnn_size, args.max_arity])
                    b = tf.get_variable('b', [args.max_arity])
                    out = tf.matmul(state, w) + b
                return out
            
            with tf.variable_scope('type_feedforward'):
                # Boolean type, number type and two refinements for string type
                self.output_b = bool_ff_network(self.state, 'bool_ff')
                self.output_n = num_ff_network(self.state, 'num_ff')
                self.output_s1 = num_ff_network(self.state, 'str1_ff')
                self.output_s2 = bool_ff_network(self.state, 'str2_ff')
