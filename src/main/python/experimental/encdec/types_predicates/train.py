    ...
    with tf.Session() as sess:
        ...
        for i in range(args.num_epochs):
            ...
            for b in range(args.num_batches):
                ...
                # setup the feed dict
                x, l, n, e, y, p = data_loader.next_batch()
                ...
                for j in range(args.num_predicates):
                    feed[model.targets_p[j].name] = p[j]

                # run the optimizer
                cost, lt, lg, _ = sess.run([model.cost, model.type_losses, model.sequence_loss,
                                            model.train_op], feed)
                end = time.time()
                step = i * args.num_batches + b
                if step % args.print_every == 0:
                    print('{}/{} (epoch {}), cost: {:.3f} ({} + {:.3f}*{}), time: {:.3f}' \
                            .format(step, args.num_epochs * args.num_batches, i, cost, lt, lg,
                                args.num_predicates, end - start))
