...
def main():
    ...
    parser.add_argument('--max_arity', type=int, default=3,
                       help='maximum arity of API methods')
    ...
    with tf.Session() as sess:
        ...
        for i in range(args.num_epochs):
            ...
            for b in range(args.num_batches):
                ...
                # setup the feed dict
                x, l, n, e, y, teb, ten, tes, tb, tn, ts1, ts2 = data_loader.next_batch()
                init_state = model.encoder.cell_init.eval()
                feed = { model.targets: y,
                         model.targets_exists_b: teb,
                         model.targets_exists_n: ten,
                         model.targets_exists_s: tes,
                         model.targets_b: tb,
                         model.targets_n: tn,
                         model.targets_s1: ts1,
                         model.targets_s2: ts2,
                ...
                # run the optimizer
                cost, lt, lb, ln, \
                ls1, ls2, lg, _ = sess.run([model.cost, model.type_loss, model.loss_b,
                                            model.loss_n, model.loss_s1, model.loss_s2,
                                            model.sequence_loss, model.train_op], feed)
                end = time.time()
                step = i * args.num_batches + b
                if step % args.print_every == 0:
                    print('{}/{} (epoch {}), cost: {:.3f} ({:.3f} ({:.3f} + {:.3f} + {:.3f} ' \
                            '+ {:.3f}) + {:.3f}), time: {:.3f}'.format(step, args.num_epochs * \
                            args.num_batches, i, cost, lt, lb, ln, ls1, ls2, lg, end - start))
